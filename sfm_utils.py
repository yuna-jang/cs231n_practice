...
class Frame:
    ...
    def __init__(self, matches, focal_length, im_width, im_height, K1, K2, R, T):
        self.focal_length = focal_length
        self.im_height = im_height
        self.im_width = im_width
        self.matches = matches
        ...
        self.K.append(K2)
        self.T = T
        # self.E = np.cross(self.T,R)
        self.RT = np.hstack((R,T))

        self.motion = np.zeros((2, 3, 4))
        self.motion[0, :, :-1] = np.eye(3)
        self.motion[1, :, :] = self.RT
        self.structure = triangulate(self)
'''
NEG_ONES
Arguments:
...
def triangulate(frame):
    ...
    for i in range(num_cameras):
        try:
            all_camera_matrices[i,:,:] = frame.K[i].dot(frame.motion[i,:,:])
        except:
            all_camera_matrices[i,:,:] = frame.K_std.dot(frame.motion[i,:,:])
...
def bundle_adjustment(frame):
    ...
    vec = least_squares(reprojection_error_mot_str_opt, np.hstack((motion_angle_axis.flatten(), frame.structure.flatten())),
        args=(frame.match_idx, frame.match_points, frame.focal_length, px, py), method='dogbox')
...
def remove_outliers(frame, threshold = 10.0):
    ...
    for i in range(frame.match_idx.shape[0]):
        X = frame.K_std.dot(transform_points(frame.structure, frame.motion[i,:,:]).T)
        xy = X[:2, :] / X[2, :]
...
def merge_all_frames(frames):
    ...
    for i in range(1,len(frames)):
        ...
        # remove_outliers(merged_frame, 100000)
        # bundle_adjustment(merged_frame)
