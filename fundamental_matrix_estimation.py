...
def lls_eight_point_alg(points1, points2):  # lecture5_epipolar p.31
    points_num = points2.shape[0]

    W = np.zeros((points_num, 9)) # 37X9
    ...
    # compute F_hat
    U, s, VT = np.linalg.svd(W, full_matrices=True) # Wf=UsVT (SVD algorithm)
    f = VT[-1, :]
    print(f)
...
def normalized_eight_point_alg(points1, points2):
    N = points1.shape[0]
    points1_uv = points1[:, 0:2] # Remove 3rd column
    ...
    # Matrix T is used for transformation(translation + scaling)
